...
function AwardModal({ teamId, teamName, awards, onClose, hackathonId }: { teamId: string; teamName: string; awards: any[]; onClose: () => void; hackathonId: string }) {
  ...
  const assignMut = useMutation({
    ...
    onSuccess: () => { toast.success('Нагороду призначено'); qc.invalidateQueries({ queryKey: ['full-results', hackathonId] }); onClose() },
    onError: () => toast.error('Помилка'),
  })
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-card border border-border rounded-xl p-6 w-full max-w-sm shadow-xl">
        <h3 className="font-semibold text-lg mb-1">Призначити нагороду</h3>
        <p className="text-sm text-muted-foreground mb-4">Команда: <span className="font-medium text-foreground">{teamName}</span></p>
        <div className="space-y-2 mb-5">
          {awards.map((a: any) => (
            <label key={a.id} className={`flex items-center gap-3 rounded-lg border p-3 cursor-pointer transition-colors ${selected === a.id ? 'border-primary bg-primary/5' : 'border-border hover:bg-muted/40'}`}>
              ...
              <span>{a.place === 1 ? '🥇' : a.place === 2 ? '🥈' : a.place === 3 ? '🥉' : '🏅'} {a.name} — {a.place} місце</span>
            </label>
          ))}
          {awards.length === 0 && <p className="text-sm text-muted-foreground text-center py-4">Немає доступних нагород. Спочатку створіть нагороди.</p>}
        </div>
        <div className="flex gap-3 justify-end">
          <button onClick={onClose} className="rounded-lg border border-border px-4 py-2 text-sm hover:bg-accent">Скасувати</button>
          <button onClick={() => assignMut.mutate()} disabled={!selected || assignMut.isPending} className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90 disabled:opacity-60">
            {assignMut.isPending ? 'Призначення...' : 'Призначити'}
          </button>
        </div>
      </div>
    </div>
...
export function ResultsPanel({ hackathonId }: Props) {
  ...
  const reinstateTeamMut = useMutation({
    ...
    onSuccess: () => { toast.success('Команду поновлено'); qc.invalidateQueries({ queryKey: ['full-results', hackathonId] }) },
    onError: () => toast.error('Помилка'),
  })
  ...
  const removeAwardMut = useMutation({
    ...
    onSuccess: () => { toast.success('Нагороду скасовано'); qc.invalidateQueries({ queryKey: ['full-results', hackathonId] }) },
    onError: () => toast.error('Помилка'),
  })
  ...
  function exportResults() {
    const rows = tracks.flatMap((t: any) =>
      t.ranked.map((e: any) => [e.position, e.teamName, t.trackName, e.project?.title ?? '—', e.normalizedTotal, e.judgeCount, e.award?.name ?? '—', e.project?.isLate ? 'Так' : 'Ні', e.project?.submittedAt ?? '—'])
    )
    exportCSV('results.csv', rows, ['Місце', 'Команда', 'Трек', 'Проєкт', 'Норм. бал', 'Суддів', 'Нагорода', 'Запізнення', 'Подано'])
  }
  ...
  function exportScores() {
    ...
    exportCSV('scores.csv', rows, ['Команда', 'Проєкт', 'Трек', 'Суддя', 'Критерій', 'Оцінка', 'Макс.', 'Вага'])
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <StatCard label="Команд" value={stats.totalTeams ?? 0} />
        <StatCard label="Схвалено" value={stats.approvedTeams ?? 0} color="text-green-600" />
        <StatCard label="Дискваліф." value={stats.disqualifiedTeams ?? 0} color="text-red-600" />
        <StatCard label="Подано проєктів" value={stats.submittedProjects ?? 0} sub={stats.lateSubmissions > 0 ? `${stats.lateSubmissions} із запізненням` : undefined} />
        <StatCard label="Середній бал" value={`${stats.averageScore ?? 0}`} color="text-primary" />
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        <button onClick={exportResults} className="flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm hover:bg-accent transition-colors">
          <Download className="h-4 w-4" /> Результати CSV
        </button>
        <button onClick={exportScores} className="flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm hover:bg-accent transition-colors">
          <Download className="h-4 w-4" /> Оцінки CSV
        </button>
        <button onClick={exportFull} className="flex items-center gap-2 rounded-lg bg-primary px-3 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors">
          <FileText className="h-4 w-4" /> Повний звіт
        </button>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden shadow-sm">
        <div className="px-5 py-3 bg-muted/40 border-b border-border flex items-center gap-2">
          ...
          <span className="font-semibold text-sm">{activeTrack?.trackName ?? 'Результати'}</span>
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="text-xs text-muted-foreground bg-muted/20 border-b border-border">
              <tr>
                ...
                {(activeTrack?.ranked?.[0]?.perJudge ?? []).map((_j: any, i: number) => (
                  <th key={i} className="px-4 py-3 text-right text-nowrap">Суддя {i + 1}</th>
                ))}
                <th className="px-4 py-3 text-right">Бал</th>
                <th className="px-4 py-3 text-center">Нагорода</th>
            ...
            <tbody className="divide-y divide-border">
              {(activeTrack?.ranked ?? []).map((e: any) => (
                <tr key={e.teamId} className="hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    ...
                    <div className="text-xs text-muted-foreground">{e.members?.length ?? 0} учасн.</div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="font-medium max-w-[180px] truncate">{e.project?.title ?? '—'}</div>
                    {e.project?.isLate && <span className="text-[10px] text-orange-500">⚠️ Запізнення</span>}
                  </td>
                  ...
                  <td className="px-4 py-3 text-center">
                    {e.award ? (
                      <button onClick={() => removeAwardMut.mutate({ teamId: e.teamId, awardId: e.award.id })}
                        className="text-xs text-red-600 hover:underline">✕ Скасувати</button>
                    ) : (
                      <button onClick={() => setAwardModal({ teamId: e.teamId, teamName: e.teamName })}
                        ...
                        <Medal className="h-3 w-3" /> Нагорода
                      </button>
                    )}
                  </td>
                </tr>
              ...
              {(activeTrack?.ranked ?? []).length === 0 && (
                <tr><td colSpan={20} className="px-4 py-10 text-center text-muted-foreground">Оцінених команд немає</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {disqualified.length > 0 && (
        <div className="rounded-xl border border-red-200 bg-card overflow-hidden shadow-sm">
          <div className="px-5 py-3 bg-red-50 border-b border-red-200 flex items-center gap-2">
            ...
            <span className="font-semibold text-sm text-red-700">Дискваліфіковані ({disqualified.length})</span>
          </div>
          <table className="w-full text-sm">
            <thead className="text-xs text-muted-foreground bg-muted/10 border-b border-border">
              <tr>
                ...
                <th className="px-4 py-2 text-left">Трек</th>
                ...
                <tr key={t.teamId} className="hover:bg-muted/20">
                  ...
                  <td className="px-4 py-3 text-center">
                    <button onClick={() => reinstateTeamMut.mutate(t.teamId)} disabled={reinstateTeamMut.isPending}
                      ...
                      <RotateCcw className="h-3 w-3" /> Поновити
                    </button>
                  </td>
                </tr>
              ))}
      ...
      {notSubmitted.length > 0 && (
        <div className="rounded-xl border border-border bg-card overflow-hidden shadow-sm">
          <div className="px-5 py-3 bg-muted/40 border-b border-border flex items-center gap-2">
            ...
            <span className="font-semibold text-sm text-muted-foreground">Не подали проєкт ({notSubmitted.length})</span>
          </div>
          <table className="w-full text-sm">
            <tbody className="divide-y divide-border">
