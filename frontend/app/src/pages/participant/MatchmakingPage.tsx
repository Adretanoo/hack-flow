...
export function MatchmakingPage() {
  ...
  // We filter out teams where member count >= max team size
  const teams = allTeams.filter((t: any) => {
    const memberCount = t._count?.members ?? t.members?.length ?? 0
    // We don't have maxTeamSize on list item directly, show all with space
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        title="Пошук команди"
        subtitle="Знайдіть команду для участі в хакатоні та подайте заявку на вступ"
      />
      ...
      <div className="relative w-full sm:w-80">
        ...
        <input
          type="text"
          placeholder="Пошук за назвою команди..."
      ...
      {teams.length === 0 ? (
        <EmptyState
          ...
          description="Спробуйте змінити пошуковий запит або поверніться пізніше"
        />
      ) : (
              ...
              >
                ...
                <div className="p-5 flex-1 space-y-3">
                  ...
                  <div className="flex items-center gap-1.5 text-xs">
                    ...
                    <span className={isFull ? 'text-red-500 font-semibold' : 'text-muted-foreground'}>
                      {memberCount}/{maxSize} учасників {isFull ? '(заповнена)' : ''}
                    </span>
                  </div>

                  {/* Status */}
                  {approvalStatus === 'APPROVED' && (
                    <span className="inline-block px-2 py-0.5 rounded text-xs bg-green-100 text-green-700">✅ Підтверджена</span>
                  )}
                </div>
                ...
                {isExpanded && (
                  <div className="px-5 pb-4 space-y-3 border-t border-border pt-4">
                    <label className="block text-sm font-medium">Повідомлення для капітана (необов'язково)</label>
                    <textarea
                      rows={2}
                      placeholder="Розкажіть про себе або свої навички..."
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        {sendRequestMut.isPending ? <LoadingSpinner size="sm" /> : <><Send className="h-3 w-3" /> Надіслати</>}
                      </button>
                    </div>
                  </div>
                )}
                ...
                <div className="border-t border-border px-5 py-3">
                  {requestStatus === 'sent' ? (
                    <div className="flex items-center gap-1.5 text-sm text-green-600 font-medium justify-center">
                      <CheckCircle className="h-4 w-4" /> Заявку надіслано
                    </div>
                  ) : requestStatus === 'error' ? (
                    <div className="flex items-center gap-1.5 text-sm text-red-500 justify-center">
                      <XCircle className="h-4 w-4" /> Помилка. Спробуйте ще раз
                    </div>
                  ) : isFull ? (
                    <div className="text-center text-xs text-muted-foreground">Команда заповнена</div>
                  ) : !user ? (
                    <div className="text-center text-xs text-muted-foreground">Увійдіть, щоб подати заявку</div>
                  ) : !isExpanded ? (
                    <button
                    ...
                    >
                      <Send className="h-3.5 w-3.5" /> Подати заявку
                    </button>
                  ) : null}
                </div>
              </div>
      ...
      <div className="rounded-xl border border-border bg-muted/20 px-5 py-4 text-sm text-muted-foreground flex items-start gap-3">
        ...
        <span>
          Після подачі заявки капітан команди отримає запит і зможе прийняти або відхилити вас.
          Слідкуйте за статусом у вкладці <b>«Моя команда»</b> в кабінеті хакатону.
        </span>
      </div>
    </div>
  )
